...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            pg_node_action = PgNode_a(action)
            if pg_node_action.prenodes.issubset(self.s_levels[level]):
                action_node_set.add(pg_node_action)
                # for each pre-node
                for pre_node in pg_node_action.prenodes:
                    pre_node.children.add(pg_node_action)
                    pg_node_action.parents.add(pre_node)

        self.a_levels.append(action_node_set)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_add_array = node_a1.action.effect_add
        a1_remove_array = node_a1.action.effect_rem
        a2_add_array = node_a2.action.effect_add
        a2_remove_array = node_a2.action.effect_rem
        for a1_add in a1_add_array:
            if a1_add in a2_remove_array:
                return True

        for a1_remove in a1_remove_array:
            if a1_remove in a2_add_array:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_effect_add_array = node_a1.action.effect_add
        a1_effect_remove_array = node_a1.action.effect_rem
        a1_pre_pos_array = node_a1.action.precond_pos
        a1_pre_neg_array = node_a1.action.precond_neg

        a2_effect_add_array = node_a2.action.effect_add
        a2_effect_remove_array = node_a2.action.effect_rem
        a2_pre_pos_array = node_a2.action.precond_pos
        a2_pre_neg_array = node_a2.action.precond_neg

        for a1_add in a1_effect_add_array:
            if a1_add in a2_pre_neg_array:
                return True

        for a1_remove in a1_effect_remove_array:
            if a1_remove in a2_pre_pos_array:
                return True

        for a2_add in a2_effect_add_array:
            if a2_add in a1_pre_neg_array:
                return True

        for a2_remove in a2_effect_remove_array:
            if a2_remove in a1_pre_pos_array:
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        actions_for_s1 = node_s1.parents
        actions_for_s2 = node_s2.parents
        for action_for_s1 in actions_for_s1:
            for action_for_s2 in actions_for_s2:
                if not action_for_s1.is_mutex(action_for_s2):
                    return False
        return True
